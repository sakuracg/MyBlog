  ...
  <div>
    ...
    <div class="all-blog-main">
      <div class="allblog-content">
        <div
          class="blog-item"
          v-for="(blog, index) in currentCollect"
          :key="'blog' + index"
        >
          <div class="img-box">
            <router-link
...
export default {
  ...
  methods: {
    getAllCollectBlog () {
      getAllArtCollect(res => {
        this.getAllCollect = this.currentCollect = res.blogCollects
        // 按照类型给博客分类进数组
        this.classifyBlogs(this.getAllCollect)
      })
    },
    showCateBlogs (type) {
      if (type === 0) {
        this.currentStyle = 0
        this.currentCollect = this.getAllCollect
      } else if (type === 1) {
        this.currentStyle = 1
        this.currentCollect = this.getTechCollect
      } else if (type === 2) {
        this.currentStyle = 2
        this.currentCollect = this.getNovelCollect
      } else if (type === 3) {
        this.currentStyle = 3
        this.currentCollect = this.getAnimeCollect
      }
    },
    classifyBlogs (blogs) {
      for (let blog of blogs) {
        if (blog.pcateId === 1) {
          this.getTechCollect.push(blog)
        } else if (blog.pcateId === 2) {
          this.getNovelCollect.push(blog)
